      ...
      <main className="flex min-h-screen flex-col items-center justify-center p-8">
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md w-full max-w-3xl">
          <div className="flex flex-col items-center mb-6">
            {dbUser?.picture ? (
              ...
              <div className="w-24 h-24 rounded-full bg-gray-300 flex items-center justify-center mb-4">
                <span className="text-2xl">{dbUser?.name?.charAt(0) || 'U'}</span>
              </div>
            )}
          ...
          <div className="space-y-4">
            ...
            <div className="p-4 bg-gray-100 dark:bg-gray-700 rounded-md">
              <h3 className="font-medium mb-2">Your Grocery Items ({groceryItems.length})</h3>
              {groceryItems.length > 0 ? (
                <ul className="list-disc pl-5">
                  {groceryItems.map((item: { id: number; name: string; quantity: number }) => (
                    <li key={item.id}>
